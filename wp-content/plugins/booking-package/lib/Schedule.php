    	...
    	public function bookingDetailsForHotel($accommodationDetails, $currency, $mode = 'array'){
			...
			$objectList = array(
				'totalLengthOfStay' => array(
					...
					#'main' => sprintf($nights, $accommodationDetails['nights'], $accommodationDetails['nights'] + 1) . " " . $this->formatCost(($accommodationDetails['accommodationFee']), $currency), 
					'sub' => array(),
				), 
					...
					for($i = 0; $i < count($guests); $i++){
						...
						if(intval($guests[$i]['selected']) == 1){
							...
							if (intval($guests[$i]['price']) > 0) {
								...
								#$details .= ", " . $this->formatCost($guests[$i]['price'], $currency) . " * " . sprintf($nights, $accommodationDetails['nights'], $accommodationDetails['nights'] + 1);
